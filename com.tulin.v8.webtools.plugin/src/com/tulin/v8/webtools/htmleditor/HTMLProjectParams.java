...
public class HTMLProjectParams {
	...
	public void load(IProject project) throws Exception {
		...
		if (configFile.exists()) {
			...
			// for old versions
			this.root = project.getPersistentProperty(new QualifiedName(HTMLPlugin.getPluginId(), P_ROOT));
			useDTD = project.getPersistentProperty(new QualifiedName(HTMLPlugin.getPluginId(), P_USE_DTD));
			validateHTML = project
					.getPersistentProperty(new QualifiedName(HTMLPlugin.getPluginId(), P_VALIDATE_HTML));
		}

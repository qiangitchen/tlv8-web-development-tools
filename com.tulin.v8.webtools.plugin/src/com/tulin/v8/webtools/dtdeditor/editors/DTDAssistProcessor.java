...
public class DTDAssistProcessor extends HTMLAssistProcessor {
	...
	private static String[] ATTR_TYPES = { "CDATA", "ID", "IDREF", "IDREFS", "ENTITY", "ENTITIES", "NMTOKEN",
			"NMTOKENS" };
		...
		if (word.startsWith("<") || context == UNDEF) {
			...
			for (String element : elements) {
				if (element.startsWith(word)) {
					list.add(createProposal(element, element, word, documentOffset, element.length(),
							HTMLPlugin.ICON_ELEMENT));
				}
			}
		} else if (context == ATTLIST_ATTRTYPE) {
			for (int i = 0; i < ATTR_TYPES.length; i++) {
				if (ATTR_TYPES[i].startsWith(word)) {
					list.add(createProposal(ATTR_TYPES[i], ATTR_TYPES[i], word, documentOffset, ATTR_TYPES[i].length(),
							HTMLPlugin.ICON_VALUE));
				}
			}
			...
			for (int i = 0; i < ATTR_OMITS.length; i++) {
				if (ATTR_OMITS[i].startsWith(word) && !word.startsWith("\"")) {
					...
					list.add(createProposal(ATTR_OMITS[i], ATTR_OMITS[i], word, documentOffset, position,
							HTMLPlugin.ICON_VALUE));
				}
			}
			...
			for (int i = 0; i < ELEM_TYPES.length; i++) {
				if (ELEM_TYPES[i].startsWith(word)) {
					...
					list.add(createProposal(ELEM_TYPES[i], ELEM_TYPES[i], word, documentOffset, position,
							HTMLPlugin.ICON_VALUE));
				}
			}
		}
	...
	private static ICompletionProposal createProposal(String text, String display, String word, int offset,
			int position, String image) {
		return new CompletionProposal(text, offset - word.length(), word.length(), position,
				HTMLPlugin.getDefault().getImageRegistry().get(image), display, null, null);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.tulin.v8.webtools.assist.HTMLAssistProcessor#enableTemplate()
	 */
	@Override

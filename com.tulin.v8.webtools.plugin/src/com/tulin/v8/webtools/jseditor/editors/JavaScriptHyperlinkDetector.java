...
import com.tulin.v8.webtools.jseditor.editors.model.JavaScriptModel.RequirePathData;
	...
	private class JavaScriptHyperlink implements IHyperlink {
		...
		public void open() {
			...
			if (!file.exists()) {
				...
				String caption = HTMLPlugin.getResourceString("ErrorDialog.Caption");
				String message = HTMLPlugin.createMessage(HTMLPlugin.getResourceString("Error.FileNotFound"),
						new String[] { file.getFullPath().toString() });
				String details = HTMLPlugin.getResourceString("Error.FileNotFoundDetails");
				IStatus status = new Status(IStatus.ERROR, HTMLPlugin.getPluginId(), 0,
						HTMLPlugin.createMessage(details, new String[] { file.getFullPath().toString() }), null);

 ...
 * 
 * @see com.tulin.v8.webtools.jseditor.editors.JavaScriptEditor
		...
		for (JavaScriptElement jsElement : jsContext.getElements()) {
			if (jsElement.isUndefined()) {
				...
				if (start >= 0) {
					...
					// .getName().length();
					String message = HTMLPlugin.createMessage(HTMLPlugin.getResourceString("Validation.UseUndefined"),

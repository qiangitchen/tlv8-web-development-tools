...
import com.tulin.v8.webtools.jseditor.rhino.javascript.RegExpProxy;
...
public class RegExpImpl implements RegExpProxy {
	...
	public int find_split(Context cx, Scriptable scope, String target, String separator, Scriptable reObj, int[] ip,
		...
		again: while (true) { // imitating C label
			...
			if (matchlen[0] == 0) {
				/*
				 * Empty string match: never split on an empty match at the start of a
				 * find_split cycle. Same rule as for an empty global match in match_or_replace.
				 */
				if (i == ip[0]) {
					/*
					 * "Bump-along" to avoid sticking at an empty match, but don't bump past end of
					 * string -- our caller must do that by adding sep->length to our return value.
					 */
					if (i == length) {
	...
	/**
	 * Analog of REGEXP_PAREN_SUBSTRING in C jsregexp.h. Assumes zero-based; i.e.,
	 * for $3, i==2
	 */
	...
	private static SubString interpretDollar(Context cx, RegExpImpl res, String da, int dp, int[] skip) {
		...
		case '`':
			if (version == Context.VERSION_1_2) {
				/*
				 * JS1.2 imitated the Perl4 bug where left context at each step in an iterative
				 * use of a global regexp started from last match, not from the start of the
				 * target string. But Perl4 does start $` at the beginning of the target string
				 * when it is used in a substitution, so we emulate that special case here.
				 */
	...
	SubString[] parens; /*
						 * Vector of SubString; last set of parens matched (perl $1, $2)
						 */

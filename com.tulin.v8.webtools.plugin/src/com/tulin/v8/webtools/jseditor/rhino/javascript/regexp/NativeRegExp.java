...
package com.tulin.v8.webtools.jseditor.rhino.javascript.regexp;
...
import com.tulin.v8.webtools.jseditor.rhino.javascript.IdFunctionObject;
import com.tulin.v8.webtools.jseditor.rhino.javascript.IdScriptableObject;
...
import com.tulin.v8.webtools.jseditor.rhino.javascript.ScriptableObject;
import com.tulin.v8.webtools.jseditor.rhino.javascript.Undefined;
...
//    private static final byte REOP_RPARENNON     = 40; /* non-capturing version of REOP_RPAREN */
	...
	/*
	 *
	 * 1. If IgnoreCase is false, return ch. 2. Let u be ch converted to upper case
	 * as if by calling String.prototype.toUpperCase on the one-character string ch.
	 * 3. If u does not consist of a single character, return ch. 4. Let cu be u's
	 * character. 5. If ch's code point value is greater than or equal to decimal
	 * 128 and cu's code point value is less than decimal 128, then return ch. 6.
	 * Return cu.
	 */
	...
	private static boolean calculateBitmapSize(CompilerState state, RENode target, char[] src, int index, int end) {
		...
		while (index != end) {
			...
			case '\\':
				...
				case '7':
					/*
					 * This is a non-ECMA extension - decimal escapes (in this case, octal!) are
					 * supposed to be an error inside class ranges, but supported here for backwards
					 * compatibility.
					 *
					 */
					n = (c - '0');
	 ...
	 * static property RegExp.multiline is true). '$' End of string (or line if the
	 * class static property RegExp.multiline is true). '\b' Word boundary (between
	 * \w and \W). '\B' Word non-boundary.
	 *
	 * quantatom: atom An unquantified atom. quantatom '{' n ',' m '}' Atom must
	 * occur between n and m times. quantatom '{' n ',' '}' Atom must occur at least
	 * n times. quantatom '{' n '}' Atom must occur exactly n times. quantatom '*'
	 * Zero or more times (same as {0,}). quantatom '+' One or more times (same as
	 * {1,}). quantatom '?' Zero or one time (same as {0,1}).
	 *
	 * any of which can be optionally followed by '?' for ungreedy
	 *
	 * atom: '(' regexp ')' A parenthesized regexp (what matched can be addressed
	 * using a backreference, see '\' n below). '.' Matches any char except '\n'.
	 * '[' classlist ']' A character class. '[' '^' classlist ']' A negated
	 * character class. '\f' Form Feed. '\n' Newline (Line Feed). '\r' Carriage
	 * Return. '\t' Horizontal Tab. '\v' Vertical Tab. '\d' A digit (same as [0-9]).
	 * '\D' A non-digit. '\w' A word character, [0-9a-z_A-Z]. '\W' A non-word
	 * character. '\s' A whitespace character, [ \b\f\n\r\t\v]. '\S' A
	 * non-whitespace character. '\' n A backreference to the nth (n decimal and
	 * positive) parenthesized expression. '\' octal An octal escape sequence (octal
	 * must be two or three digits long, unless it is 0 for the null character).
	 * '\x' hex A hex escape (hex must be two digits). '\c' ctrl A control
	 * character, ctrl is a letter. '\' literalatomchar Any character except one of
	 * the above that follow '\' in an atom. otheratomchar Any character not first
	 * among the other atom right-hand sides.
	 */
	...
	private static boolean parseTerm(CompilerState state) {
		...
		case '\\':
			if (state.cp < state.cpend) {
				...
				case '0':
					 ...
					 * However, (XXX and since Rhino doesn't have a 'strict' mode) we'll just behave
					 * the old way for compatibility reasons. (see
				...
				case '9':
					...
					if (num > state.parenCount)
						reportWarning(state.cx, "msg.bad.backref", "");
					/*
					 * n > 9 or > count of parentheses, then treat as octal instead.
					 */
		...
		case '[':
			...
			/*
			 * Call calculateBitmapSize now as we want any errors it finds to be reported
			 * during the parse phase, not at execution.
			 */
		...
		case '{': /* balance '}' */
		{
			...
			/*
			 * For Perl etc. compatibility, if quntifier does not match \{\d+(,\d*)?\}
			 * exactly back off from it being a quantifier, and chew it up as a literal atom
			 * next time instead.
			 */
	 ...
	 * CharacterSetMatcher(A, false) and return its Matcher result. 6. E must be an
	 * integer. Let n be that integer. 7. If n=0 or n>NCapturingParens then throw a
	 * SyntaxError exception. 8. Return an internal Matcher closure that takes two
	 * arguments, a State x and a Continuation c, and performs the following: 1. Let
	 * cap be x's captures internal array. 2. Let s be cap[n]. 3. If s is undefined,
	 * then call c(x) and return its result. 4. Let e be x's endIndex. 5. Let len be
	 * s's length. 6. Let f be e+len. 7. If f>InputLength, return failure. 8. If
	 * there exists an integer i between 0 (inclusive) and len (exclusive) such that
	 * Canonicalize(s[i]) is not the same character as Canonicalize(Input [e+i]),
	 * then return failure. 9. Let y be the State (f, cap). 10. Call c(y) and return
	 * its result.
	 */
	...
	/*
	 * Initialize the character set if it this is the first call. Test the bit - if
	 * the ^ flag was specified, non-inclusion is a success
	 */
	...
	private static boolean executeREBytecode(REGlobalData gData, char[] chars, int end) {
		...
		for (;;) {
			...
			/*
			 * If the match failed and there's a backtrack option, take it. Otherwise this
	...
	Object executeRegExp(Context cx, Scriptable scopeObj, RegExpImpl res, String str, int indexp[], int matchType) {
		...
		if (matchType == TEST) {
			/*
			 * Testing for a match and updating cx.regExpImpl: don't allocate an array
			 * object, do return true.
			 */
			result = Boolean.TRUE;
			obj = null;
		} else {
			/*
			 * The array returned on match has element 0 bound to the matched string,
			 * elements 1 through re.parenCount bound to the paren matches, an index
			 * property telling the length of the left context, and an input property
			 * referring to the input string.
			 */
		...
		if (!(matchType == TEST)) {
			/*
			 * Define the index and input properties last for better for/in loop order (so
			 * they come after the elements).
			 */
		...
		if (cx.getLanguageVersion() == Context.VERSION_1_2) {
			/*
			 * JS1.2 emulated Perl4.0.1.8 (patch level 36) for global regexps used in scalar
			 * contexts, and unintentionally for the string.match "list" psuedo-context. On
			 * "hi there bye", the following would result:
			 *
			 * Language while(/ /g){print("$`");} s/ /$`/g perl4.036 "hi", "there"
			 * "hihitherehi therebye" perl5 "hi", "hi there" "hihitherehi therebye" js1.2
			 * "hi", "there" "hihitheretherebye"
			 *
			 * Insofar as JS1.2 always defined $` as "left context from the last match" for
			 * global regexps, it was more consistent than perl4.
			 */
